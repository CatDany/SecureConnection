...
public class SCLog
{
	...
	public static void init(Level logLevel)
	{
		...
		logFile = new File(logFolder, Helper.dateFormatFile.format(Calendar.getInstance().getTime()) + ".txt");
		try
		{
	...
	/**
	 * Make a {@link SCException} and {@link #t(Exception) print stacktrace}
	 * @param format
	 * @param args
	 */
	public static void t(String format, Object... args)
	{
		SCException t = new SCException(format, args);
		t(t);
	}
	...
	public static void log(Level level, String format, Object... args)
	{
		...
		else if (level.value <= logLevel.value)
		{
			String s = String.format("[Log] [%s] [%s] [%s] %s",
					Helper.dateFormatVersion.format(Calendar.getInstance().getTime()),
	 ...
	 * Use {@link SCLog#log(Level, String, Object...)} instead if you want to log properly.
	 */
